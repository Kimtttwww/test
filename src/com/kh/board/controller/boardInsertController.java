...
public class boardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * form 태그의 전송방법을 multipart/form-data로 전송하는 경우
		 * requset로부터 값을 뽑는게 불가능
		 * 
...
//			1. 전송되는 파일을 처리할 작업내용(전송되는 파일의 용량제한, 전달된 파일을 저장할 폴더 경로)
//			1_1. 전송파일 용량제한(byte 단위로 제시)
			/*
			 * byte -> Kbyte -> Mbyte -> Gbyte -> Tbyte
			 */
			int maxSize = 10 * 1024 * 1024;	// 10Mega byte;
			
//			1_2. 전달된 파일을 저장할 서버의 폴더경로 알아내기
//			어플리케이션 객체에서 제공하는 getRealPath 메소드를 통해 알아내기
//			매개변수로 board_upfiles 폴더까지의 경로를 제시해줘야 한다
...
//			System.out.println(savePath);
			 ...
			 * 
			 * 매개변수 생성자로 MultipartRequest 객체를 생성(cos.jar에서 제공하는 객체)
			 * MultipartRequest multiRequest = new MultipartRequest(request 객체, 저장할 폴더 경로, 용량제한, 인코딩값, 파일명을 수정시켜주는 객체)
			 * 
			 * 위 구문 한줄 실행만으로 넘어온 첨부파일이 해당 폴더에 업로드됨
			 * 그리고 사용자가 올린 파일명을 그대로 해당 폴더에 업로드 하지 않는다(같은 파일명이 있을 경우 덮어 씌워질 수 있고, 서버에 따라 문제가 발생할 수도 있다
			 * 
			 * cos.jar에서 제공하는 파일명 수정 작업을 해주는 객체
			 * - DefaultFileRenamePolicy 객체
			 * - 내부적으로 rename()이라는 메서드가 실행이 되면서 수정이 진행됨
			 * - 작동방식은 동일한 파일명이 존재할 경우 카운팅된 숫자를 붙여서 파일명을 수정해줌
			 * ex) aaa.jpg, aaa1.jpg, aaa2.jpg
			 * 
			 * - 우리 입맛대로 파일명이 겹치지 않게끔 rename 메서드를 오버라이딩 해줄 예정
			 */
...
//			첨부파일은 필수가 아니기 때문에 우선 null로 초기화, 등록한 첨푸파일이 있을 경우 객체 생성 예
			Attachment at = null;
//			multiRequest.getOriginalFileName("키");
//			첨부파일이 있었을 경우 원본 파일명, 없었을경우 null값이 반환
...
//			5. 응답화면 지정
			...
			} else {	// 실패시 -> 첨부파일이 있었을 경우 이미 업로드된 첨부파일을 서버에서 삭제 후, 에러페이지로 포워딩
				if(at != null) {
//					삭제시키고자 하는 파일 객체 생성 후 delete 메서드 호출
