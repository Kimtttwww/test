...
//				첨부파일이 이미 존재 했을 경우(fileNo가 hidden으로 넘어왔음
				if(multi.getParameter("originFileNo") != null) {
//					새로운 첨부파일이 있던 케이스 => update attachment 실행
//					update 하기 위해서는 기존의 파일 고유번호가 필요함
					at.setFileNo(Integer.parseInt(multi.getParameter("originFileNo")));
					
//					기존의 첨부파일 삭제
					new File(savePath + multi.getParameter("originName")).delete();
				} else {
//					새롭게 첨부파일을 등록하려고 하지만, 기존에는 첨부파일이 없었을 경우 => insert Attachment
//					insert 하기 위해서는 현재 게시글 번호가 필요함
...
//			case3 : 새로운 첨부파일 O, 기존 첨부파일 X => b, refBNo가 담긴 at => Board Update, Attachment Update
